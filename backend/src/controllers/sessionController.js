...
export async function createSession(req, res) {
    try {
        ...
        // Generate a Stream user token for this user
        const streamToken = streamClient.createToken(clerkId);

        // Connect user to Stream Video and Chat clients (idempotent)
        if (!videoClient.user || videoClient.user.id !== clerkId) {
            await videoClient.connectUser({ id: clerkId }, streamToken);
        }
        if (!streamClient.user || streamClient.user.id !== clerkId) {
            await streamClient.connectUser({ id: clerkId }, streamToken);
        }

        //create stream video calling
        await videoClient.call("default", callId).getOrCreate({
            data: {
...
export async function endSession(req, res) {
    try {
        ...
        try {
            await videoClient.call("default", session.callId).delete({ hard: true });
        } catch (err) {
            console.error("Error deleting Stream video call:", err.message);
        }
        ...
        try {
            ...
            console.error("Error deleting Stream chat channel:", err.message);
            // Optionally, return a more specific error to the client here
        }
