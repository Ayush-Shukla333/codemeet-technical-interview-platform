...
function SessionPage() {
    ...
    const { streamClient, call, chatClient, channel, isInitializingCall } = useStreamClient(session, loadingSession, isHost, isParticipant);
    ...
    //auto-join session if user is not the host and there is a participant slot available
    useEffect(() => {
        if (!session || !user || loadingSession) return;
        if (isHost || isParticipant || loadingSession) return;
    ...
    }, [session, user, loadingSession, id, isHost, isParticipant]);
    ...
    useEffect(() => {
        ...
        if (session.status === "completed") {
            alert("Session has ended. You will be redirected to the dashboard.");
            navigate("/dashboard");
        }
